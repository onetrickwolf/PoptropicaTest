package net.richardlord.asteroids
{
	...
	import net.richardlord.asteroids.graphics.Starfield;
	...
	{	
		...
		public function createStarfield(x:Number, y:Number) : Entity
		{
			var starfield : Starfield = new Starfield();
			
			var starfieldEntity : Entity = new Entity( "starfield" )
				.add( new Display( starfield ) )
				.add( new Position( x, y, 0 ) )
				.add( new Motion( 0, 0, 0, 15 ) )
				.add( new MotionControls( Keyboard.LEFT, Keyboard.RIGHT, Keyboard.UP, -1, 0, false ) );
			engine.addEntity( starfieldEntity );
		...
		public function createAsteroid( radius : Number, x : Number, y : Number, velocity:Point = null) : Entity
		{
			...
			if(velocity == null)
			{
				motion = new Motion( ( Math.random() - 0.5 ) * 4 * ( 50 - radius ), ( Math.random() - 0.5 ) * 4 * ( 50 - radius ), Math.random() * 2 - 1, 0 );
			}
			else
			{
				motion = new Motion( velocity.x * Math.random(), velocity.y * Math.random(), Math.random() * 2 - 1, 0 );
			}
			
			fsm.createState( "alive" )
				.add( Motion ).withInstance( motion )
				//.add( Motion ).withInstance( new Motion( 5, 5, 2, 15 ) )
				.add( MotionControls ).withInstance( new MotionControls( Keyboard.LEFT, Keyboard.RIGHT, Keyboard.UP, -1, 0, false ) )
		...
		public function createSpaceship() : Entity
		{
			...
			fsm.createState( "playing" )
				.add( Motion ).withInstance( new Motion( 0, 0, 0, 15 ) )
				.add( MotionControls ).withInstance( new MotionControls( Keyboard.LEFT, Keyboard.RIGHT, Keyboard.UP, 0, 3, true ) )
